...
class DeepResearchContext:
    ...
    async def save_round_results(
        ...
        logger.info(
            f"Saved round {self.research_round} results for task {self.task_id}"
        )
    ...
    async def get_research_gap_analysis(self) -> str:
        ...
        if self.contradictions_found:
            gaps.append(
                f"Found {len(self.contradictions_found)} contradictions needing investigation"
            )
...
async def deep_researcher_node(
    ...
    try:
        ...
        logger.info(
            f"[{agent_name}] Running {num_sub_agents} sub-agents in parallel..."
        )
        ...
        try:
            ...
            logger.warning(
                f"[{agent_name}] Sub-agent research timed out; collecting partial results"
            )
            state.errors.append(
                "Deep research timed out after 10 minutes; using partial results"
            )
        ...
        logger.info(
            f"[{agent_name}] Sub-agents found {len(all_sources_found)} total sources"
        )
        ...
        if context.research_round < context.max_rounds:
            ...
            logger.info(
                f"[{agent_name}] Generated {len(follow_up_queries)} follow-up queries"
            )
        ...
        if context.research_round < context.max_rounds and context.contradictions_found:
            logger.info(
                f"[{agent_name}] Starting Research Round 3 (controversy resolution)"
            )
...
Return ONLY the JSON array, no other text."""
        ...
        try:
            ...
            if json_start >= 0 and json_end > json_start:
                ...
                logger.warning(
                    f"Sub-agent {agent_id}: Could not extract JSON from response"
                )
                return {
                    "agent_id": agent_id,
                    "sources": [],
...
async def _generate_controversy_queries(
    ...
    disagreement_summary = "\n".join(
        f"- {c.get('description', 'Unknown contradiction')}" for c in contradictions[:5]
    )
